...
public interface BorrowingRepository extends JpaRepository<Borrowing, Long> {
        ...
        @Query("SELECT b FROM Borrowing b " + "JOIN FETCH b.member m " + "JOIN FETCH b.bookCopy bc "
                        + "JOIN FETCH bc.book bk ORDER BY b.id DESC")
        List<Borrowing> findAllWithRelations();

        // Tìm 1 phiếu mượn cụ thể của 1 bản sao sách đang nằm ở trạng thái
        ...
        @Query("SELECT b FROM Borrowing b " + "JOIN FETCH b.member m " + "JOIN FETCH b.bookCopy bc " +
                        "JOIN FETCH bc.book bk " + "WHERE b.status = 'ACTIVE' AND b.dueDate < :today ORDER BY b.id DESC")
        List<Borrowing> findOverdueBorrowings(@Param("today") LocalDate today);

        // Lấy danh sách phiếu mượn của một thành viên.
        @Query("SELECT b FROM Borrowing b WHERE b.member.id = :memberId ORDER BY b.id DESC")
        List<Borrowing> findByMemberId(@Param("memberId") Long memberId);

        // Lấy danh sách phiếu mượn theo trạng thái.
        @Query("SELECT b FROM Borrowing b WHERE b.status = :status ORDER BY b.id DESC")
        ...
        @Query("SELECT b FROM Borrowing b " + "JOIN FETCH b.member m " + "JOIN FETCH b.bookCopy bc "
                        + "JOIN FETCH bc.book bk " + "WHERE b.member.id = :memberId ORDER BY b.id DESC")
        ...
        @Query("SELECT b FROM Borrowing b " + "JOIN FETCH b.member m " + "JOIN FETCH b.bookCopy bc "
                        + "JOIN FETCH bc.book bk " + "WHERE b.status = 'ACTIVE' AND b.dueDate < :today")
        ...
        @Query("SELECT b FROM Borrowing b " + "JOIN FETCH b.member m " + "JOIN FETCH b.bookCopy bc "
                        + "JOIN FETCH bc.book bk " + "WHERE bc.id = :copyId ORDER BY b.borrowDate DESC")
        ...
        // Lấy ngày phải trả sớm nhất theo từng đầu sách trong một danh sách
        @Query("SELECT b.bookCopy.book.id, MIN(b.dueDate) FROM Borrowing b " + "WHERE b.bookCopy.book.id IN :bookIds "
                        + "AND b.status = 'ACTIVE' " + "GROUP BY b.bookCopy.book.id")
        ...
        @Query("SELECT br.id AS borrowingId, m.id AS memberId, m.name AS memberName, m.phone AS memberPhone, " +
                        ...
                        + "FROM Borrowing br " + "JOIN br.member m " + "JOIN br.bookCopy bc " +
                        "JOIN bc.book bk " + "WHERE br.status = 'ACTIVE' AND br.dueDate < CURRENT_DATE " +
